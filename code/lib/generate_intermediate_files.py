...
def generate_transmission(paths, param):
    ...
    assumptions_tra.sort_index(inplace=True)

    # Classify line lengths according to the values in length_limit_km in df_joined
    for line_type, line_length in assumptions_try.index:
    ...
    df_joined.join(assumptions_tra, on=["Type", "length_limit_km"], how="left")

    # Add attributes
    df_completed = df_joined.copy()
    df_completed["eff"] = df_completed["eff_per_1000km"] ** (df_completed["length"] / 1000)
    df_completed["inv-cost"] = float(df_completed["inv-cost-length"]) * df_completed.loc[ind, "length"] + float(dict_costs["inv-cost-fix"])
    df_completed["fix-cost"] = float(df_completed["fix-cost-length"]) * df_completed.loc[ind, "length"]
