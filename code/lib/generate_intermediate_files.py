...
def generate_sites_from_shapefile(paths, param):
    ...
    create_json(paths["sites_sub"], param, ["region_name", "subregions_name", "Crd_all", "res_desired", "GeoRef"],
                paths, ["LAND", "EEZ", "spatial_scope", "subregions"])
    timecheck('End')
...
def generate_load_timeseries(paths, param):
    ...
    if not os.path.exists(paths["stats_countries"]):
        df = zonal_stats(param["regions_land"],
                         {'Population': paths["POP"], 'Landuse': paths["LU"]},
                         param)
    ...
    if not os.path.exists(paths["stats_country_parts"]):
        df = zonal_stats(reg_intersection,
                         {'Population': paths["POP"], 'Landuse': paths["LU"]},
                         param)
        ...
        create_json(paths["stats_country_parts"], param, ["region_name", "subregions_name", "landuse_types"], paths, ["spatial_scope", "LU", "POP", "Countries", "subregions"])
    else:
    ...
    create_json(paths["load_regions"], param, ["region_name", "subregions_name", "load", "landuse_types"], paths, ["spatial_scope", "LU", "POP", "Countries", "subregions"])
...
def generate_transmission(paths, param):
    ...
    Region_start = gpd.GeoDataFrame(grid_cleaned[['l_id', 'V1']], geometry='V1', crs={'init': 'epsg:4326'}).rename(
        columns={'V1': 'geometry'})
    Region_start.crs = subregions.crs
    Region_start = gpd.sjoin(Region_start, subregions, how='left', op='intersects')[
        ['NAME_SHORT']].rename(columns={'NAME_SHORT': 'Region_start'})
    
    # Create a dataframe for the end regions
    Region_end = gpd.GeoDataFrame(grid_cleaned[['l_id', 'V2']], geometry='V2', crs={'init': 'epsg:4326'}).rename(
        columns={'V2': 'geometry'})
    Region_end.crs = subregions.crs
    Region_end = gpd.sjoin(Region_end, subregions, how='left', op='intersects')[
